...
\[
\C(t)=\big\{\sum_{i=a}^b a_it^i\quad\text{ such that }a\leq b \in \Z\big\}
\]
of ``polynomials with finitely many negative powers'', as sets they don't coincide. Another example comes from the field
\[
\Q(i)=\{z \in \C : \Re z \in \Q, \Im z\in\Q\}
\]
which is isomorphic to $\Frac (\Z[i])$, but not literally equal to it. A definition like
%%\begin{lstlisting}
