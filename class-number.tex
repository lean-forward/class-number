...
The notion which is pivotal to the definition of the ideal class group of a Dedekind domain is that of \emph{fractional ideals}: given any integral domain $R$ with a field of fractions $F$, an 
%$R$-ideals divided by some $x : R$,
%or equivalently 
$R$-submodule $J$ of $F$ is \lean{is\_fractional} if there is an $x : R$ with $x J \subseteq R$. %The reason for introducing them is that, unlike their subset of proper ideals, 
%Recall that, f
