...
Making this an \lean{instance} instead of a \lean{lemma} ensures that Lean can now automatically "infer" a PID to be a Dedekind domain.  
The Noetherian property of a Dedekind domain follows easily by the lemma \lean{principal\_ideal\_ring.is\_noetherian\_ring}, since, by definition, each ideal is finitely generated (by a single element). In fact, this holds true even if A is a PIR instead of a PID.
...
The final condition of a PID being integrally closed is the most challenging. We show that a PID is a unique factorization domain (UFD), and that every UFD is integrally closed.  
