...
An alternative characterization of the conjugates of $x$ is that they are the images of $x$ under each embedding $K(x) \to F$ that fixes $K$.
...
lemma power_basis.trace_gen_eq_sum_embeddings [is_separable K L]
  ...
  algebra_map K F (trace K L pb.gen) =
    ∑ σ in (@finset.univ _
      ...
      σ pb.gen
\end{lstlisting}
Taking the power basis of $K(x)$ generated by $x$, we generalize the above to all $x$:
\begin{lstlisting}
...
\end{lstlisting}
In particular, if we choose $x = \lean{pb.gen}^{i + j}$, we get:
\begin{lstlisting}
lemma trace_form_gen_pow_gen_pow
  ...
  ((pb.minpoly_gen.map (algebra_map K F)).roots
    .map (λ x, x ^ (i + j))).sum
\end{lstlisting}
% TODO: explain that polynomial.roots is a multiset, hence the different notation

If we compute the matrix corresponding to the bilinear form \lean{trace\_form},
in the basis \lean{pb}, the $(i, j)$'th entry is exactly \lean{trace\_form K L (pb.gen \pow i) (pb.gen \pow j)}.
Since \lean{algebra\_map K F} is an injective ring homomorphism,
the trace form has nonzero determinant if and only if the matrix with entries
\lean{algebra\_map K F (trace\_form K L (pb.gen \pow i) (pb.gen \pow j))}
has nonzero determinant.
These entries are exactly the left hand side of \lean{trace\_form\_gen\_pow\_gen\_pow}.

Now consider the matrix $A$ where the $(i, j)$'th entry is the $i$'th conjugate of \lean{pb.gen},
to the power $j$, so if $x_k : F$ is the $k$'th conjugate of \lean{pb.gen}, we have $A_{ij} = x_i^j$.
...
so $\det A = \prod_{i < j} (x_i - x_j)^2 \ne 0$.
Then the $(i, j)$'th entry of $A^T A$ is equal to $\sum_k x_k^{i + j}$, exactly the right hand side of \lean{trace\_form\_gen\_pow\_gen\_pow}.
Therefore, the (image in $F$ of the) determinant of the trace form is the product $\det (A^T) \det A = (\det A)^2 \ne 0$, exactly what we wanted to prove.
