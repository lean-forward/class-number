...
A \emph{number field} $K$ is a finite extension of $\Q$, and as such has the structure of a finite dimensional vector space over $\Q$; it's dimension is called the \emph{degree} of $K$.
...
For an interesting cubic example, let $\alpha$ be the unique real number satisfying $\alpha^3 + \alpha^2 - 2\alpha + 8=0$, then this gives rise to the number field
...
and up to isomorphism these are all number fields of degree $n$.
...
It can be proved that the nonzero fractional ideals of $\OK$ again form a group. 
...
and can e.g. be found in~\cite{Ireland-Rosen}. For the formalization in Lean, we adapted and generalized this approach, so that on the one hand most ingredients became more \mathlib ``friendly'' (by e.g. avoiding unnecessary field extensions, as used for example in norm estimates), and on the other hand it works in the function field setting as well.
