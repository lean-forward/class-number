...
There are multiple structures possible for
Lean does not enforce uniqueness of typeclass instances, but the \mathlib maintainers try to ensure all instances that can be inferred are definitionally equal.
...
In the definition used by \mathlib, a fraction map is a special case of a \emph{localization map}. Different localizations restrict the denominators to different submonoids of $R^\times$.
...
\end{itemize}
Note that fields are Dedekind domains accorindg to these conventions.
We used \lean{is\_dedekind\_domain} and \lean{is\_dedekind\_domain\_inv} in this project.
...
we could easily replace usages of \lean{is\_dedekind\_domain\_inv R} with \lean{is\_dedekind\_domain D}.
...
On the other hand, we formalized that this equality holds for Dedekind domains (Section~\ref{sec:equivalence}) in the following:
\begin{lstlisting}
