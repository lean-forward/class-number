...
We remark that it is possible to get rid of the \lean{[is\_separable f.codomain K]} assumption in the function field case,
since any function field is actually isomorphic to a finite \emph{and separable} extension of a field isomorphic to $\Fq[q](t)$; see for example \cite[Corollary 4.4 in Chapter VIII]{Lang}.
A trivial way to get rid of this assumption in the finiteness statement above is to simply move it to our definition of function field.
While this would be mathematically consistent by the result just cited above, we did not opt to do this since it would simply move the proof obligation to the points at which \lean{is\_function\_field} gets instantiated,
for instance when showing a finite extension of a function field is a function field.
