...
We use the proof given in Cassels and Frohlich's Algebraic Number Theory (Chapter 1, Section 2, Proposition 1) % TODO:cite
...
%\lean{h₁ : (submodule.span R {x} * (1 / ↑I : fractional_ideal f).val) ≤
%(1 / ↑I : fractional_ideal f).val}.
This follows from repeated usage of the lemma \lean{submodule.mem\_div\_iff\_forall\_mul\_mem} and \lean{fractional\_ideal.coe\_div}. The latter statement requires $I$ and $J$ to be non-zero. 
=======
We consider, for a non-zero ideal $I$, the ideal $J := I * (1/I)$, and show that $1/J \leq 1 \le J$, hence $J = 1$ since $J \le 1$ holds in an arbitrary domain. So, we want to show that any element $x \in 1/J$ is in $R$, or equivalently, since we have \lean{is\_dedekind\_domain R}, it suffices to prove that $x$ is in the integral closure of $R$. We consider $A := R[x]$, and show that $A \leq 1/I$, which is Noetherian, hence $A$ is a finitely generated subalgebra containing $x$. It suffices to prove that for every $n \in \mathbb{N}$, $x^n \in 1/I$. This follows from \lean{h₁ : (submodule.span R {x} * (1 / ↑I : fractional\_ideal f).val) ≤
      (1 / ↑I : fractional\_ideal f).val}. This follows from repeated usage of the lemma \lean{submodule.mem\_div\_iff\_forall\_mul\_mem} and \lean{fractional\_ideal.coe\_div}. The latter statement requires $I$ and $J$ to be non-zero. 
>>>>>>> 7a0dab11f13dff3ac50d310ebebecb3f5b9ae0fa
...
It suffices to show that $S$ is contained in a finitely generated $R$-module $M$,
...
let $b : \iota \to L$ be a finite $K$-basis of $L$ such that each $b_i$ is integral
(which exists since any finite set $s \subseteq L$ can be multiplied by some $x \ne 0 : K$ to find a set of integral elements $x \cdot s$),
...
and we conclude that $S$ is contained in a finitely generated $R$-module, hence Noetherian.
...
the map $\lean{lmul} = \lambda x y : S, xy$ is $R$-linear map in both $x$ and $y$, so it can be represented for a fixed $x$ as a \emph{multiplication matrix} $M_x \in \mathrm{M}(R)$.
We defined the algebra trace $\Tr_{S / R}$ as an $R$-linear map from $S$ to $R$ as the composition of $\lean{lmul}$ and the trace operator for linear maps, that sums the entries along the diagonal of a matrix representation\footnote{Although the matrix representation depends on the choice of a basis for $S$ over $R$, the trace does not.}:
\begin{lstlisting}
...
{ bilin := λ x y, trace R S (x * y), .. /- proofs omitted -/ }
\end{lstlisting}

Although the trace form can be defined for any algebra,
for simplicity of exposition we will only consider finite field extensions in this paper. %% We are only considering trace forms of finite field extensions in this paper.
...
%\end{lstlisting}
We formulate the lemma in terms of the power basis, since we will need to use it for $K(x)$ and for an arbitary finite separable extension $L / K$ later in the proof.

The elements of \lean{(pb.minpoly\_gen.map (algebra\_map K F)).roots} are called \emph{conjugates} of $x$.
Each conjugates of $x$ is integral since it is a roots of (the same) monic polynomial,
...
Let $x_0, \dots, x_{n - 1} : F$ enumerate the roots of the minimal polynomial of $x$ (formalized as a vector \lean{fin n → F});
...
the conjugates of $x$ in $F$ are the images (counted with multiplicity) of $x$ under each embedding $K(x) \to F$ that fixes $K$.
...
Mapping \lean{trace\_eq\_sum\_roots} through the equivalence gives:
\begin{lstlisting}
lemma power_basis.trace_gen_eq_sum_embeddings [is_separable K L]
  ...
  algebra_map K F (trace K L pb.gen) =
    ∑ σ in (@finset.univ _
      ...
      σ pb.gen
\end{lstlisting}
Since $\sigma\ x^{i + j} = (\sigma\ x)^{i + j}$,
we can directly go from the conjugates of $x^{i + j}$ to the $i + j$'th powers of conjugates of $x$, and the result follows.
