...
The fields $K$ and $L$ are represented by arbitrary types
and the hypothesis ``$L$ is a field extension of $K$'' is represented by an instance parameter \lean{[algebra K L]}.
...
We formalize this notion in Lean by defining a map \lean{to\_principal\_ideal f : units f.codomain → units (fractional\_ideal f)}, %sending $x : K^\times$ to the principal ideal $\langle x \rangle$,
