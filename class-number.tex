...
We can also take $\alpha : L$ and $K(\alpha)$ to be the smallest subfield of $L$ containing $\alpha$ and the image of $K$; then $L = K(\alpha)$ means that $K(\alpha)$, as a subfield of $L$, is equal to the subfield $\top$ containing all elements of $L$.
We could also view $K$, $L$ and $K(\alpha)$ as subfields of an arbitrary larger field $F$.
Because $\alpha$ is algebraic the smallest subring containing $\alpha$ and $K$ will be a field, thus we can add two more representations, replacing ``smallest subfield'' with ``smallest subring''.
...
we chose to formalize the notion of \emph{power basis} to represent simple field extensions, a basis of the form $1, x, x^2, \dots, x^{n-1} : L$ (viewing $L$ as a $K$-vector space)\footnote{In the formalization we generalize this notion to any algebra $A$ over a commutative ring $R$}.
We call $x$ the \emph{generator} and $n$ the \emph{dimension} of this power basis.
We defined the following structure, bundling the information of a power basis:
\begin{lstlisting}
structure power_basis (K L : Type*) [field K] [field L] [algebra K L] :=
(gen : S) (dim : ℕ)
(is_basis : is_basis K (λ (i : fin dim), gen ^ (i : ℕ)))
\end{lstlisting}
...
or being able to choose the $\alpha$ generating $K(\alpha)$ (by setting \lean{power\_basis.gen\ pb} equal to $\alpha$).
Specializing a result from an arbitrary $L$ with a power basis over $K$,
to a specific value of $L$ such as $K(\alpha) = \lean{algebra.adjoin K \{α\}}$, is a matter of applying the result to the power basis generated by $\alpha$, and rewriting $\lean{power\_basis.gen (adjoin.power\_basis K $\alpha$)} = \alpha$.
...
Broadly speaking, one could see the formalization work as part of number theory. There are several formalization result in this direction; see e.g.~\cite[Section 6]{CapSetProblem}, most notably the formalization of a substantial part of analytic number theory~\cite{Eberl19}.
