...
Our work is developed as part of the mathematical library \mathlib~\mathlib\cite{mathlib} for the Lean 3 theorem prover~\cite{lean-prover}.
...
The theorem \lean{fractional\_ideal.mul\_inv\_cancel} proves the converse. The proof goes along these lines : We differentiate into cases when the Dedekind domain $A$ is and isn't a field. This is done because we require that every non-zero ideal must contain a product of non-zero prime ideals (\lean{exists\_prime\_spectrum\_prod\_le\_and\_ne\_bot\_of\_domain}), and fields don't have non-zero prime ideals. The field case is trivial, since the only non-zero fractional ideal in a field is the field itself (\lean{fractional\_ideal.eq\_zero\_or\_one\_of\_is\_field}). When $A$ is not a field, most books show that it is sufficient to look at maximal ideals of $A$, but we show something more general. We show that it is sufficient to prove invertibility for non-zero ideals of $A$. This is done in the lemma \lean{coe\_ideal\_mul\_one\_div}. This lemma is proved by contradiction. We consider, for a non-zero ideal $I$, an ideal $J := I * (1/I)$, and show that $1/J \leq 1$, hence $J = 1$. 
